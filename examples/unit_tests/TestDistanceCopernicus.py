...
class TestDistanceCopernicus(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_raster_data['data'][3:6, 3:6] = 1
        ...
        self.distance_copernicus = DistanceCopernicus(
            self.mock_raster_data, 
            self.mock_vector_traffic_area
        )

    @patch('greento.distance.DistanceCopernicus.ox')
    @patch('greento.distance.DistanceCopernicus.GeoUtils')
    @patch('greento.distance.DistanceCopernicus.cKDTree')
    @patch('greento.distance.DistanceCopernicus.nx')
    @patch('greento.distance.DistanceCopernicus.tqdm')
    def test_get_nearest_green_position(self, mock_tqdm, mock_nx, mock_cKDTree, mock_GeoUtils, mock_ox):
        ...
        mock_GeoUtils.return_value = mock_geo_utils_instance
        
        with patch('rasterio.transform.xy', return_value=([10.35, 10.45], [49.65, 49.55])):
            
            with patch.object(self.distance_copernicus, 'get_nearest_green_position', 
                              return_value=(50.3, 10.3)):
                
                lat, lon = 50.2, 10.2
                result = self.distance_copernicus.get_nearest_green_position(lat, lon)
    ...
    def test_directions(self, mock_tqdm, mock_ox):
        ...
        with patch.object(self.distance_copernicus, 'directions', 
            ...
            self.distance_copernicus.preprocessed_graph = None
            result = self.distance_copernicus.directions(lat1, lon1, lat2, lon2, transport_mode)
