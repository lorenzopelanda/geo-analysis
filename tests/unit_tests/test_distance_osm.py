...
@pytest.fixture
def distance_osm(mock_osm_green, mock_vector_traffic_area):
    """Fixture per creare un'istanza di osm."""
    return osm(mock_osm_green, mock_vector_traffic_area)
...
def test_get_nearest_green_position(mock_tqdm, mock_nx, mock_cKDTree, mock_geo, mock_ox, distance_osm):
    ...
    with patch('rasterio.transform.xy', return_value=([10.35, 10.45], [49.65, 49.55])):
        with patch.object(distance_osm, 'get_nearest_green_position', return_value=(50.3, 10.3)):
            lat, lon = 50.2, 10.2
            result = distance_osm.get_nearest_green_position(lat, lon)
...
@patch('greento.distance.osm.ox')
@patch('greento.distance.osm.tqdm')
def test_directions(mock_tqdm, mock_ox, distance_osm):
    ...
    with patch.object(distance_osm, 'directions', return_value=json.dumps({"distance_km": 1.2, "estimated_time_minutes": 30})):
        lat1, lon1 = 50.1, 10.1
        lat2, lon2 = 50.5, 10.5
        transport_mode = "walk"

        distance_osm.preprocessed_graph = None
        result = distance_osm.directions(lat1, lon1, lat2, lon2, transport_mode)
