...
class ghspop(interface):
    ...
    def __get_tiles_for_bounds(
        self, bounds: "boundingbox", tiles_gdf: gpd.GeoDataFrame
    ) -> List[str]:
        """
        Gets the tile IDs that intersect with the given bounding box.
    ...
    def __merge_tiles(
        self, tiles: List[Tuple[np.ndarray, Affine, str, Tuple[int, int]]]
            ...
            for tile_id in tile_ids:
                ...
                if tile_data:
                    ...
                    processed_paths.append(
                        os.path.join(self.extracted_dir, f"GHS_POP_{tile_id}.tif")
                    )
    ...
    def get_data(
        ...
        steps = [
            ...
            "Loading shapefile",
            "Getting tiles for bounds",
            "Downloading and processing tiles",
            "Cropping bounds",
        ]
        with tqdm(
            total=100, desc="Overall Progress", unit="steps", leave=False
        ) as pbar:
            ...
            ghs_data, ghs_transform, ghs_crs, ghs_shape = (
                self.__download_and_process_tiles(tiles_to_download)
            )
            pbar.update(50)
            pbar.set_description(steps[4])
            (
                ghs_data_cropped,
                ghs_transform_cropped,
                ghs_crs_cropped,
                ghs_shape_cropped,
            ) = self.__crop_bounds(ghs_data, ghs_transform, bounding_box)
            pbar.update(10)
            ...
            pbar.close()
            return {
                "data": ghs_data_cropped,
                "transform": ghs_transform_cropped,
                "crs": ghs_crs_cropped,
                "shape": ghs_shape_cropped,
            }
