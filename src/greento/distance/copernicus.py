...
class copernicus(interface):
    ...
    def get_nearest_green_position(self, lat: float, lon: float) -> Tuple[float, float]:
        ...
        with tqdm(total=100, desc="Processing nearest green position", leave=False) as pbar:
            ...
            pbar.update(10)
            distances_euclidian = [geo().haversine_distance(lat, lon, gp_lat, gp_lon) for gp_lat, gp_lon in green_coords]
            pbar.update(10)
        ...
        with tqdm (total=100, desc="Calculating the direction", leave=False) as pbar:
            ...
            if not hasattr(self, "preprocessed_graph"):
                ...
                if isinstance(G, tuple) and len(G) == 2:
                    ...
                    logger.error("Unexpected vector_traffic_area type")
                    return None
            ...
            for node_id, data in G.nodes(data=True):
                if "x" not in data or "y" not in data:
                    logger.error(f"Node {node_id} does not have 'x' and 'y' coordinates")
                    return None
